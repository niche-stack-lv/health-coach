...
export default function BookCallPage() {
  ...
  const generateConfirmation = () => {
    generateConfirmationPDF({
      ...
      price: formatPrice(config.pricing.callPrice),
      type: "call",
    }).catch(console.error);
  };
  ...
  const handleConfirm = async () => {
    setSubmitting(true);
    await saveLead({ source: "book-call", name, phone, email, plan_label: `Phone Consultation (${config.pricing.callDuration})`, price: formatPrice(config.pricing.callPrice) });
    ...
    const msg = [
      ...
      `*Phone:* ${phone}`,
      `*Email:* ${email}`,
      `*Service:* Phone Consultation (${formatPrice(config.pricing.callPrice)})`,
      ``,
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="max-w-lg mx-auto px-4 py-6 pb-12 space-y-6">
        <div className="rounded-xl border border-gold/20 bg-gold/5 p-5 text-center">
          ...
          <p className="text-4xl font-black text-gradient-gold mt-4">{formatPrice(config.pricing.callPrice)}</p>
